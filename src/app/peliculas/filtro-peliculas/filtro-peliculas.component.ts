...
export class FiltroPeliculasComponent implements OnInit {
  ...
  buscarPeliculas(valores: FiltroPeliculas){
    ...
    if (valores.proximoEstrenos){
      this.peliculas = this.peliculas.filter(peliculas => peliculas.proximosEstrenos);
    }
  ...
  escribirParametrosBusquedaEnURL(valores: FiltroPeliculas){
    ...
    if(valores.proximoEstrenos){
      queryStrings.push(`proximoEstrenos=${valores.proximoEstrenos}`);
    }
  ...
  limpiar(){
    this.form.patchValue({titulo: '', generoId: 0, proximoEstrenos: false, enCines: false});
  }
